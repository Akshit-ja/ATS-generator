...
BASE_URL = "http://localhost:8000"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"❌ Admin usage stats failed: {response.status_code} - {response.text}")
                return False
...
class APITester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    ...
                    print(f"❌ Create budget settings failed: {response.status_code}")
                    return False
            else:
                print(f"❌ Get budget settings failed: {response.status_code}")
                return False
        except Exception as e:
    ...
    def run_all_tests(self):
        ...
        # User registration (might fail if user exists, that's ok)
